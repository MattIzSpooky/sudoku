...
namespace Domain.Parsers
{
    public class SamuraiSudokuParser : NormalSudokuParser
    {
        ...
        {
            var cleanedContent = Regex.Replace(content.Replace(Environment.NewLine, string.Empty).Trim(), @"\s+", "");
        ...
        private static void MergeOverflowingQuadrants(IReadOnlyList<Field> grids, int amountOfCellsPerSubSudoku)
        {
            ...
            for (var i = 0; i < grids.Count; i++)
            {
                if (i != centerGrid)
                {
                    ...
                    var secondQuadrant = grids[centerGrid].Quadrants[centerQuadrant].Children.OfType<CellLeaf>()
                        .ToImmutableList();
        ...
        private static void MergeOverflowingCells(IReadOnlyList<CellLeaf> firstCells,
            IReadOnlyList<CellLeaf> secondCells)
        {
